...
class AdapterFixedCastLocationOfWork(AdapterFixedCastABC):
...
class AdapterFixedCastLocationPlanPeriod(AdapterFixedCastABC):
...
class AdapterFixedCastEvent(AdapterFixedCastABC):
...
class DlgFixedCast(QDialog):
    ...
    def __init__(self, parent: QWidget, adapter: AdapterFixedCastABC):
        ...
        self.object_with_fixed_cast = adapter.object_with_fixed_cast.model_copy()
    ...
    def date_changed(self):
        self.persons = self.adapter.method_date_changed(self.de_date.date().toPython())
        print(f'date_changed: {self.de_date.date()=}')
        self.lb_warning.setText(self.adapter.warning_text)
