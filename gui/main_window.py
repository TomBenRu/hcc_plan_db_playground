    ...
    def __init__(self, app: QApplication, screen_width: int, screen_height: int):
        ...
        self.actions = {
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'folder-open-table.png'),
                              self.tr('Öffenen... (Planung)'),
                              self.tr('Öffnet Planungsdaten für Locations und Mitarbeiter einer bestimmten Planungsperiode'),
                              self.open_plan_period_masks),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'blue-document--plus.png'),
                              self.tr('Neue Planung...'),
                              self.tr('Legt eine neue Planung an.'), self.new_plan_period, short_cut='Ctrl+n'),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'blue-document--pencil.png'),
                              self.tr('Planung ändern...'),
                              self.tr('Ändern oder Löschen einer vorhandenen Planung.'), self.edit_plan_period),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'gear--pencil.png'),
                              self.tr('Projekt-Einstellungen...'),
                              self.tr('Bearbeiten der Grundeinstellungen des Projekts'), self.settings_project),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'folder-open-document.png'),
                              self.tr('Öffnen... (Pläne)'),
                              self.tr('Öffnet einen bereits erstellten Plan.'), self.open_plan),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'disk.png'),
                              self.tr('Plan speichern...'),
                              self.tr('Speichert den aktiven Plan'),
                              self.plan_save),
            MenuToolbarAction(self, None,
                              self.tr('Excel-Einstellungen'),
                              self.tr('Einstellungen der Farben in der exportierten Excel-Datei...'),
                              self.plan_excel_configs),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'tables.png'),
                              self.tr('Listen für Sperrtermine erzeugen...'),
                              self.tr('Erstellt Listen, in welche Mitarbeiter ihre Sperrtermine eintragen können.'),
                              self.sheets_for_availables),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'download-mac-os.png'),
                              self.tr('Daten von Online-API importieren...'), 
                              self.tr('Import von Daten aus der API'),
                              self.import_from_plan_api),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'table-export.png'), 
                              self.tr('Plan Excel-Export...'),
                              self.tr('Exportiert den aktiven Plan in einer Excel-Datei'), 
                              self.plan_export_to_excel),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'blue-folder-open-table.png'),
                              self.tr('Öffnen des Excel-Export-Ordners'),
                              self.tr('Pläne der aktuellen Planungen und Verfügbarkeiten werden im Explorer angezeigt.'),
                              self.lookup_for_excel_plan_folder),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'cross-script.png'), 
                              self.tr('Programm beenden'),
                              None, self.exit,
                              'Alt+F4'),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'address-book-blue.png'), 
                              self.tr('Stammdaten...'),
                              self.tr('Stammdaten von Mitarbeitern und Einsatzorten bearbeiten.'), 
                              self.master_data),
            MenuToolbarAction(self, None, self.tr('Pläne anzeigen'), None, self.show_plans),
            MenuToolbarAction(self, None, self.tr('Masken anzeigen'), None, self.show_masks),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'table-select-cells.png'),
                              self.tr('Übersicht Verfügbarkeiten Excel-Export'),
                              self.tr('Exportiert eine Übersicht der Verfügbarkeiten der Mitarbeiter in dieser Planung.'),
                              self.export_avail_days_to_excel),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'chart.png'),
                              self.tr('Übersicht Einsätze der Mitarbeiter'),
                              self.tr('Zeigt eine Übersicht der Einsätze aller Mitarbeiter*innen an.'), 
                              self.statistics),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'blue-document-attribute-p.png'),
                              self.tr('Einsatzpläne erstellen...'),
                              self.tr('Einen oder mehrere Einsatzpläne einer bestimmten Periode erstellen.'),
                              self.calculate_plans),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'notebook--pencil.png'), 
                              self.tr('Plan-Infos...'),
                              self.tr('Erstellt oder ändert Infos zur Planung.'), 
                              self.plan_infos),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'gear.png'),
                              self.tr('Einstellungen für Plan-Erstellung...'), 
                              self.tr('Einstellungen für die Berechnung der Pläne.'),
                              self.plan_calculation_settings),
            MenuToolbarAction(self, None, self.tr('Excel Output-Ordner...'),
                              self.tr('Ordner für die Ausgabe von Excel-Files festlegen'), 
                              self.determine_excel_output_folder),
            MenuToolbarAction(self, None, self.tr('Pläne des aktuellen Teams endgültig löschen...'),
                              self.tr('Die zum Löschen markierten Pläne des aktuellen Teams werden endgültig gelöscht'),
                              self.plans_of_team_delete_prep_deletes),
            MenuToolbarAction(self, None, self.tr('Events aus Plan zu Events-Maske...'),
                              self.tr('Termine aus aktivem Plan in Planungsmaske Einrichtungen übernehmen.'),
                              self.apply_events__plan_to_mask),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'mail-send.png'), 
                              self.tr('Emails...'), 
                              self.tr('Emails versenden.'),
                              self.send_email),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'calendar--arrow.png'), 
                              self.tr('Termine übertragen'),
                              self.tr('Termine des aktiven Plans zum Google-Kalender übertragen'),
                              self.plan_events_to_google_calendar),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'calendar--plus.png'),
                              self.tr('Google-Kalender anlegen'),
                              self.tr('Einen neuen Google-Kalender anlegen'), 
                              self.create_google_calendar),
            MenuToolbarAction(self, None, self.tr('Lokale Kalenderliste abgleichen'),
                              self.tr('Gleicht die lokale Kalenderliste mit den online verfügbarn Google-kalendern ab'),
                              self.synchronize_google_calenders),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'calendar-blue.png'),
                              self.tr('Google-Kalender öffnen...'),
                              self.tr('Öffnet den Google-Kalender im Browser'), 
                              self.open_google_calendar),
            MenuToolbarAction(self, None, self.tr('Google-API Credentials importieren...'), 
                              self.tr('Kalender-ID des Google-Kalenders dieses Teams'),
                              self.import_google_api_credentials),
            MenuToolbarAction(self, None, self.tr('Upgrade...'), 
                              self.tr('Zum Erweitern von "hcc-plan"'), 
                              self.upgrade_hcc_plan),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'gear--pencil.png'),
                              self.tr('Allg. Programm-Einstellungen'), 
                              self.tr('Allgemeine Programmeinstellungen.'),
                              self.general_setting),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'book-question.png'), 
                              self.tr('Hilfe...'),
                              self.tr('Öffnet die Hilfe im Browser.'), 
                              self.open_help, 'F1'),
            MenuToolbarAction(self, None, self.tr('Auf Updates überprüfen...'),
                              self.tr('Überprüft, ob Updates zum Programm vorhanden sind.'), 
                              self.check_for_updates),
            MenuToolbarAction(self, os.path.join(path_to_toolbar_icons, 'information-italic.png'), 
                              self.tr('Über...'),
                              self.tr('Info über das Programm'), 
                              self.about_hcc_plan)
        }
        ...
        self.menu_actions = {
            self.tr('&Datei'): [self.actions['open_plan_period_masks'], self.actions['new_plan_period'],
                       self.actions['edit_plan_period'], None, self.actions['sheets_for_availables'], None,
                       self.actions['import_from_plan_api'], None,
                       {
                           self.tr('Exportieren'): [self.actions['plan_export_to_excel'],
                                           self.actions['export_avail_days_to_excel']]
                        },
                       self.actions['lookup_for_excel_plan_folder'],
                       None, self.actions['exit']],
            self.tr('&Teams'): [{self.tr('Teams bearbeiten'): [self.put_teams_to__teams_edit_menu]}, None, self._put_clients_to_menu,
                          None, self.actions['master_data']],
            self.tr('&Ansicht'): [{'toggle_plans_masks': (self.actions['show_plans'], self.actions['show_masks'])},
                         self.actions['statistics']],
            self.tr('&Spielplan'): [self.actions['calculate_plans'], self.actions['plan_infos'],
            ...
            self.tr('&Google Kalender'): [self.actions['plan_events_to_google_calendar'], self.actions['open_google_calendar'],
            ...
            self.tr('&Hilfe'): [self.actions['open_help'], None, self.actions['check_for_updates'], None,
