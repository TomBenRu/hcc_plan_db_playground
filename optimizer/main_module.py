...
from optimizer.fitness_avaluation import fitness_of_plan_period_cast__date_cast, \
    fitness_of_plan_period_cast__time_of_day_cast
...
def switch_avail_days__date_cast(plan_period_cast: PlanPeriodCast, nr_random_appointments: int,
    ...
    for appointment in random_appointments:
        try:
            ...
            controller.execute(pop_out_pop_in_commands.DateCastPopOutAvailDay(plan_period_cast, appointment, avail_day_to_pop))
    ...
    for appointment in modified_appointments:
        ...
        controller.execute(pop_out_pop_in_commands.DateCastPutInAvailDay(plan_period_cast, appointment, avail_day_to_put_in))
...
def optimize_plan_period_cast(plan_period_cast: PlanPeriodCast, nr_random_appointments: int):
    ...
    nr_iterations = 0
    # curr_fitness = fitness_of_plan_period_cast__date_cast(plan_period_cast)
    curr_fitness = fitness_of_plan_period_cast__time_of_day_cast(plan_period_cast)
    while True:
        controller = command_base_classes.ContrExecUndoRedo()

        # switch_avail_days__date_cast(plan_period_cast, nr_random_appointments, controller)
        switch_avail_days__time_of_day_cast(plan_period_cast, nr_random_appointments, controller)

        nr_iterations += 1

        # fitness_new = fitness_of_plan_period_cast__date_cast(plan_period_cast)
...
if __name__ == '__main__':
    ...
    # initial_fitness = fitness_of_plan_period_cast__date_cast(initial_cast)
